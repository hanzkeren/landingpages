...
type Category = 'Technology' | 'Business' | 'Health & Wellness' | 'Productivity' | 'Innovation';
      ...
      <div className="container">
        <div className="mb-16 text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-lg text-muted-foreground">
            Stay updated with the latest insights, trends, and tips across various topics to keep
            ahead of the curve.
          </p>
        </div>
        <div className="mx-auto max-w-7xl">
          <div className="my-16 grid grid-cols-1 items-center gap-8 md:grid-cols-2 lg:gap-16">
            ...
            <div className="flex flex-col items-start gap-4">
              ...
              <p className="text-muted-foreground md:max-w-lg">{posts[0].description}</p>
            </div>
          </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-10 md:grid-cols-3 md:gap-6">
            {posts.slice(1).map((post) => (
              <div key={post.title} className="flex flex-col items-start gap-4">
                ...
                <h3 className="text-xl font-semibold text-balance md:max-w-md">{post.title}</h3>
                <p className="text-muted-foreground md:max-w-md">{post.description}</p>
              </div>
            ))}
          </div>
        </div>
