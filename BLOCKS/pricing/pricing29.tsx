...
const ITEMS: PricingTier[] = [
  ...
  },
  {
    ...
    features: ['All basic plan features and...', 'Advanced security controls', 'Migration support'],
    cta: {
      text: 'Book a demo',
      href: '/',
    },
  ...
  return (
    <section className="relative py-32 text-center">
      <div className="container">
        <h1 className="text-4xl font-semibold tracking-tight md:text-5xl lg:text-6xl">Pricing</h1>
        ...
        <div className="relative mt-8 md:mt-12 lg:mt-20">
          ...
          <div className="relative space-y-6 md:grid md:grid-cols-3 md:space-y-0 md:divide-x md:divide-background/20 md:p-6 lg:p-8">
            {ITEMS.map((tier, index) => (
              <PricingCard key={tier.name} tier={tier} isHighlighted={index === 1} />
            ))}
          </div>
        </div>
...
function PricingCard({ tier, isHighlighted }: { tier: PricingTier; isHighlighted: boolean }) {
  const styles = {
    card: cn(
