...
const Integration11 = () => {
  ...
  if (tab === 'disconnected') filtered = INTEGRATIONS.filter((i) => !toggles[i.id]);
      ...
      <div className="container">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="mb-8">
            {TABS.map((t) => (
              <TabsTrigger key={t.value} value={t.value} className="px-4 py-2 text-sm font-medium">
                {t.label}
              </TabsTrigger>
            ))}
          </TabsList>
            ...
            <TabsContent key={t.value} value={t.value}>
              {grouped.map((group) => (
                <div key={group.category} className="mb-10">
                  <h2 className="mb-1 text-lg font-semibold">{group.category}</h2>
                  <p className="mb-4 text-sm text-muted-foreground">{group.categoryDescription}</p>
