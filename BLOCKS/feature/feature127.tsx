...
import { Blocks, ChevronRight, Infinity as InfinityIcon, Laptop, ListEnd } from 'lucide-react';
      ...
      <div className="container max-w-[1200px]">
        <div className="flex gap-6">
          <div className="w-full">
            <span className="text-base text-muted-foreground">Build with blocks</span>
            <h2 className="my-4 text-3xl font-medium">What you can do with our utilities</h2>

            <p className="text-lg text-muted-foreground">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptate qui dignissimos
              odit.
            </p>
          </div>
          ...
          <div className="relative hidden w-full lg:block">
            ...
            <div className="grid w-full grid-cols-8 grid-rows-4 gap-2">
              {Array.from({ length: 32 - gridItems.length }).map((_, index) => (
                <div key={index} className="flex w-full rounded-md bg-muted p-3" />
              ))}
              {gridItems.map((item) => (
                <div
                  key={item.id}
                ...
                >
                  <img src={item.imgSrc} alt="placeholder" className="h-full w-full object-cover" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-2 grid grid-cols-1 gap-6 md:grid-cols-2">
          {itemsData.map((item) => (
            <div key={item.title} className="rounded-lg border p-12 pb-0">
              ...
              <p className="font-base mb-8 text-muted-foreground">{item.description}</p>
