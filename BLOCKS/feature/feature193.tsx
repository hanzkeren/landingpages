...
const items = [
  {
    ...
    className: 'flex flex-col pl-6 py-6 overflow-hidden md:col-span-3 md:flex-row gap-6 md:gap-12',
    ...
    <section className="relative overflow-hidden py-32">
      ...
      <div className="relative z-10 container mt-8 grid grid-cols-1 gap-6 md:grid-cols-5">
        {items.map((item, index) => (
          <Card key={index} className={cn('col-span-1 shadow-xl', item.className)}>
            <CardHeader className={item.headerClassName}>
              ...
              {item.description.map((desc, i) => (
                <CardDescription key={i} className="mt-3 text-base leading-snug font-medium">
                  {desc}
                </CardDescription>
              ))}
            </CardHeader>
            <CardContent className={item.contentClassName}>
              {item.imagePosition === 'content' && (
                <img
                  src={item.image.src}
                  alt={item.image.alt}
                  className={cn('self-center object-cover', item.image.className)}
                />
              )}
            </CardContent>
          </Card>
