...
const ITEMS: ItemType[] = [
  {
    ...
    image: {
      ...
      className: 'lg:translate-x-20 translate-x-6 md:translate-x-10 rounded-b-none',
    },
    fade: ['right'],
  },
  {
    ...
    >
      <CardHeader className="mb-2">
        <h3 className="inline leading-tight font-semibold text-balance">
          {title} <span className="font-medium text-muted-foreground">{description}</span>
        </h3>
      </CardHeader>

      <CardContent className={cn('relative flex overflow-hidden p-0', containerClassName)}>
        {fade.includes('right') && (
        ...
        <img
          src={image.src}
          alt={image.alt}
          className={cn('flex-1 rounded-xl border object-cover', image.className)}
        />
      </CardContent>
    </Card>
  );
