...
import { GlowingStarsBackgroundCard } from '#app/components/aceternity/glowing-stars';
  ...
  return (
    <section className="bg-background text-foreground dark overflow-hidden py-32">
      <div className="container flex w-full flex-col items-center justify-center px-4">
        <p className="bg-muted mb-4 rounded-full px-2 py-1 text-xs uppercase">INTEGRATIONS</p>
        ...
        <div className="mt-10 grid w-full max-w-5xl grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          {cardData.map((card, index) => (
            <div key={index} className="bg-muted/60 flex flex-col rounded-3xl p-4">
              <GlowingStarsBackgroundCard className="h-55 border-none !bg-none" />

              <div className="mt-3 flex items-center justify-start gap-3">
                <h3 className="text-2xl font-semibold tracking-tighter">{card.title}</h3>
              ...
              <div className="mt-5 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <h3 className="-mt-1 text-2xl font-semibold tracking-tight">{card.price}</h3>
                </div>
              </div>
            </div>
          ))}
