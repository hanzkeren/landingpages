    ...
    <section className="py-32">
      <div className="container">
        ...
        <div className="mt-12 grid grid-cols-1 gap-10 sm:grid-cols-2">
          {items1.map((item) => (
            <div className="overflow-hidden rounded-2xl bg-muted pb-8" key={item.title}>
              <img src={item.imgSrc} alt="placeholder" className="h-40 w-full object-cover" />
              <p className="mt-8 mb-4 px-10 text-2xl font-semibold">{item.title}</p>
              <p className="px-10 text-base text-muted-foreground">{item.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12">
          {items2.map((item, index) => (
            ...
            >
              ...
              <p className="w-full text-base text-muted-foreground">{item.description}</p>
            </div>
          ))}
        </div>
      </div>
