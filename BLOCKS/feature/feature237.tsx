...
const CARDS: CardData[] = [
  ...
  },
  {
    ...
    description: 'Zero-trust security model with biometric authentication and behavioral analysis.',
    href: '#security',
    metrics: {
      value: '0.001%',
  ...
  },
  {
    ...
    description: 'High-precision geolocation services with indoor mapping and spatial analytics.',
    href: '#location',
    metrics: {
      value: '±0.5m',
      label: 'Accuracy',
                ...
                <div className="flex h-full flex-col justify-between space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="flex items-center space-x-2">
                          <card.status.icon className={cn('h-4 w-4', card.status.color)} />
                          <span className={cn('font-mono text-sm', card.status.color)}>
                            {card.status.label}
                          </span>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="font-mono text-xl font-bold">{card.metrics.value}</div>
                        ...
                        <div
                          className={cn(
                            'font-mono text-sm',
                            card.metrics.trend > 0 ? 'text-green-500' : 'text-red-500'
                          )}
                        >
                    ...
                    <div className="grid grid-cols-3 gap-2 border-t pt-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="font-mono text-xs">{card.performance.cpu}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Server className="h-4 w-4 text-muted-foreground" />
                        <span className="font-mono text-xs">{card.performance.memory}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Zap className="h-4 w-4 text-muted-foreground" />
                        <span className="font-mono text-xs">{card.performance.latency}</span>
                      </div>
                    </div>
                    ...
                    <div className="border-t pt-4">
                      <div className="flex items-center space-x-2 font-mono text-xs text-muted-foreground">
                        ...
                        <span>•</span>
                        <span className="uppercase">{card.deployment.environment}</span>
                        <span>•</span>
