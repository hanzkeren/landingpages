...
const FeatureItem = ({ title, summary, icon: Icon }: Item) => {
  return (
    <div className="flex flex-col items-center justify-center gap-4">
      ...
      <div className="text-muted-foreground max-w-sm text-center text-base">{summary}</div>
    </div>
  );
};
          ...
          <div className="flex flex-col gap-16">
            <div className="mx-auto max-w-4xl text-center">
              ...
              <div className="text-lg font-medium tracking-tight text-white">
                Accessibility ensures digital content is usable by all, including people with
                disabilities, by following inclusive design practices and meeting standards like
                WCAG for equal user experience.
              </div>
            </div>
            ...
            <div className="grid items-stretch gap-8 lg:grid-cols-3">
              {FEATURES.map((item) => (
                ...
                >
                  ...
                  <div className="max-w-sm text-center text-sm text-white/70">{item.summary}</div>
                </div>
              ))}
            </div>
          </div>
