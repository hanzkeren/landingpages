...
import {
  ...
  TreeProvider,
  TreeView,
} from '#app/components/kibo-ui/tree';
    ...
    return (
      <TreeNode key={node.id} nodeId={node.id} level={level} isLast={isLast}>
        ...
        {hasChildren && node.children && (
          <TreeNodeContent hasChildren>{renderTreeNodes(node.children, level + 1)}</TreeNodeContent>
        )}
      </TreeNode>
    );
  });
...
const Codeexample4 = () => {
  const [selectedFileId, setSelectedFileId] = React.useState<string>('button.tsx');
    ...
    <section className="py-32">
      <div className="container">
        <div className="mb-12 text-center">
          <span className="text-muted-foreground mb-6 text-xs uppercase">Component Library</span>
          <h2 className="mb-4 text-4xl">Rich, reusable components</h2>
          <p className="text-muted-foreground mx-auto max-w-3xl text-lg">
            Discover our comprehensive collection of production-ready components. Browse through the
            file tree to explore different implementations, patterns, and best practices for
          ...
          <div className="min-h-0 min-w-0 overflow-hidden">
            ...
            >
              <CodeBlockHeader className="justify-between">
                <CodeBlockFilename value={selectedFile?.language || 'typescript'}>
